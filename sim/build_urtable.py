...
# Wrist camera (one per arm): an ArduCam OV9782 board on each gripper, looking over
# the fingers at the grasp. The camera is defined directly in the Robotiq XML
# (body "wrist_cam" under "base"); tune its pose there. See gripper/robotiq-2f85.xml.
...
# The two arms face each other: they share the same lift/elbow/wrist angles but
# differ in shoulder_pan so each reaches toward the table center. The left arm is
# rotated 180deg (reaches +x) and the right arm -90deg relative to the base pan.
HOME_POSE = {
    "left_":  [1.57 + np.pi,      -1.134, 1.134, -1.5708, -1.5708, 0.0],
    "right_": [1.57 + np.pi / 2,  0, 1.134, -1.5708, -1.5708, 0.0],
}
...
# Robot pose is set per-joint by name (see set_initial_pose) from the per-arm
# HOME_POSE, so it survives layout changes (added gripper/object joints). Edit
...
def set_initial_pose(model: mujoco.MjModel, data: mujoco.MjData) -> None:
    ...
    for prefix in ("left_", "right_"):
        pose = HOME_POSE[prefix]          # per-arm pose; each faces the table center
