...
removes it; the poses are unchanged.
    ...
    python bimanual_test.py --scene full
"""
...
def print_table(results: dict[str, dict]) -> None:
    """Each scene is scored over its own tabletop, so a waypoint missing from a
    scene's grid prints as '-' rather than being dropped from the table."""
    scenes = list(results)
    names = list(dict.fromkeys(n for s in scenes for n in results[s]))
    ...
    for name in names:
        line = f"{name:<22}"
        for s in scenes:
            line += f"{results[s].get(name, {}).get('verdict', '-'):>{w}}"
        print(line)
    print("-" * len(header))
    print(f"{'PASS':<22}" + "".join(
        f"{sum(1 for r in results[s].values() if r['verdict'] == 'PASS'):>{w}}" for s in scenes))
    print(f"{'   of':<22}" + "".join(f"{len(results[s]):>{w}}" for s in scenes))
    print()
    for s in scenes:
