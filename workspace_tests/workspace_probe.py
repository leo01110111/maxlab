...
Opens a scene (build_urtable / build_urtable_45 / build_urgantry) in the MuJoCo
...
blue = collision -- and "replay sweep" drives the arm through the clean arc.
    ...
    python workspace_probe.py --scene urtable
    python workspace_probe.py --scene urgantry --report      # headless, no GUI
    python workspace_probe.py --scene urtable --sweep        # report + orientation sweeps

Coordinates are the scene frame from build_urtable.py: origin at the center of
the table footprint on the floor, +x right, +y back, +z up, meters.
"""
...
SCENES = {
    "urtable": "build_urtable.py",
    "urtable_45": "build_urtable_45.py",
    "urgantry": "build_urgantry.py",
}
...
# Meters between the two arms' targets in "both" mode. 0.35 is the smallest value
# that keeps the grippers clear of each other over the table center in urtable,
# whose bases sit only 20 cm apart on the front edge; the splayed (30 cm) and
# gantry (31 cm, elevated) layouts are clean well below it.
...
class Probe:
    ...
    def _build_panel(self, board):
        tk = self.tk
        hl, hw = self.mod.HALF_LEN, self.mod.HALF_W
        for label, var, lo, hi, res in (
            ("x  (left/right)", self.x, -hl, hl, 0.005),
            ("y  (front/back)", self.y, -hw, hw, 0.005),
