...
Writes one mp4 per scene plus a side-by-side of all three, so the layouts can be
compared on the same motion. Scene setup matches bimanual_test.py -- props removed,
implicitfast integrator.
  ...
  split   each arm follows its OWN sampled list of waypoints, independently and
          ...
          The same two lists are replayed on all three layouts.
...
# Every layout mounts its arms on the front (-y) edge, so the camera sits on the
# opposite (+y) edge looking back across the table at them -- from the front the arms
# and the gantry column stand between the lens and the work area.
CAM = {"azimuth": 270.0, "elevation": -24.0, "distance": 2.35, "lookat_y": -0.12}
FONT_PATH = "/usr/share/fonts/truetype/dejavu/DejaVuSansMono.ttf"

# 450 waypoints at 1 s each is a 7.5 minute clip, so the shared-mode tour samples the
# grid rather than walking all of it. The step between successive waypoints is random
# in [1, 12] instead of fixed, so the tour mixes 5 cm nudges with long reaches instead
# of only ever showing one travel distance. The test itself still uses every waypoint.
STRIDE_MIN, STRIDE_MAX = 1, 12
SPLIT_N = 40
...
def random_stride(waypoints: list[dict], lo: int, hi: int, seed: int) -> list[dict]:
    """Walk the serpentine order in random increments. Seeded, so all three layouts
...
def sample_lists(waypoints: list[dict], n: int, seed: int) -> dict[str, list[dict]]:
    ...
    all three layouts are driven through identical lists."""
    rng = np.random.default_rng(seed)
...
def _setup(scene: str):
    ...
    camera.lookat[:] = [0.0, CAM["lookat_y"], mod.BOARD_TOP + 0.05]
...
def combine(paths: list[Path], out_path: Path) -> None:
    """Side-by-side triptych. The runs are frame-aligned (same waypoints, same dwell),
    so column i of every panel is the same target."""
    readers = [imageio.get_reader(p) for p in paths]
    writer = imageio.get_writer(out_path, fps=FPS, macro_block_size=1)
    try:
        for frames in zip(*readers):
            writer.append_data(np.hstack(frames))
    finally:
        writer.close()
        for r in readers:
            r.close()
...
def main():
    ...
    for scene in scenes:
        ...
        paths.append(out)
    if len(paths) > 1:
        combined = OUT_DIR / f"all_scenes{suffix}.mp4"
