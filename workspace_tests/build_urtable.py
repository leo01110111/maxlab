...
# Arm base placement: both arms on the front long edge, 12 cm in from that
# edge, 20 cm apart center-to-center, straddling the table's midline (x=0) so
# the midpoint between the bases sits on the midpoint of the long edge.
ARM_EDGE_DIST = 0.12     # in from the front (long) edge
ARM_SEPARATION = 0.20    # center-to-center distance between the two bases
ARM_Y = -HALF_W + ARM_EDGE_DIST                         # -0.45
ARM_LEFT = (-ARM_SEPARATION / 2, ARM_Y)                 # (-0.10, -0.45)
ARM_RIGHT = (ARM_SEPARATION / 2, ARM_Y)                 # (+0.10, -0.45)
...
# Base yaw: 180deg about z so both arms (at pan=0) face +y, into the table from
# the front edge. Quat [w, x, y, z] for a rotation of theta about +z.
ARM_BASE_YAW = np.pi
ARM_BASE_QUAT = [np.cos(ARM_BASE_YAW / 2), 0.0, 0.0, np.sin(ARM_BASE_YAW / 2)]
LEFT_BASE_QUAT = ARM_BASE_QUAT
RIGHT_BASE_QUAT = ARM_BASE_QUAT
...
def build_spec() -> mujoco.MjSpec:
    ...
    # --- black board on top, cut around the two plates -----------------------
    # Both plates sit together near the front edge, so the cutout is a single
    # notch spanning both of them; the rest of the board wraps around it.
    board_z = TABLE_H + BOARD_T / 2
    notch_x0, notch_x1 = ARM_LEFT[0] - plate_half, ARM_RIGHT[0] + plate_half
    notch_y1 = ARM_Y + plate_half
    pieces = [
        ("front_left", (-HALF_LEN, notch_x0), (-HALF_W, notch_y1)),
        ("front_right", (notch_x1, HALF_LEN), (-HALF_W, notch_y1)),
        ("back", (-HALF_LEN, HALF_LEN), (notch_y1, HALF_W)),
    ]
